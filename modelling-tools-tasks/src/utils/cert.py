...
from src.integrations.modelling_tools_api.codegen import Database
from src.utils.validators import get_bool_from_str_or_bool, validate_path


def get_clickhouse_verify_by_database(tenant_id: str, database: Database) -> bool | str:
    """
    ...
    """
    db_verify_key = f"DB_{tenant_id}_{database.name}_VERIFY".upper()
...
def get_certs_by_database(
    tenant_id: str, database: Database, is_semantic: bool = False, node_label: str = ""
) -> tuple[
    Optional[str],
    Optional[str],
    Optional[str],
    ...
    if not is_semantic:
        ca_cert_var = f"DB{node_label}_{tenant_id}_{database.name}_PATH_TO_CA_CERT".upper()
        client_cert_var = f"DB{node_label}_{tenant_id}_{database.name}_PATH_TO_CLIENT_CERT".upper()
        client_cert_key_var = f"DB{node_label}_{tenant_id}_{database.name}_PATH_TO_CLIENT_CERT_KEY".upper()
        client_cert_password_var = f"DB{node_label}_{tenant_id}_{database.name}_CLIENT_CERT_PASSWORD".upper()
    else:
...
def get_ssl_mode_by_database(
    tenant_id: str, database: Database, is_semantic: bool = False, node_label: str = ""
) -> Optional[str]:
    """Возвращает sslmode для базы данных"""
    if not is_semantic:
        ssl_mode = f"DB_{tenant_id}_{database.name}_CONNECTION_{node_label}_SSLMODE".upper()
    else:
        node_label = "" if node_label == "0" else node_label
...
def get_ssl_context_by_database(
    ...
    ca_cert, client_cert, client_cert_key, client_cert_password = get_certs_by_database(
        tenant_id, database, is_semantic, node_label
    )
...
def get_cert_and_verify_for_httpx(
    ...
    cert: Optional[str | tuple[str, ...]] = None
